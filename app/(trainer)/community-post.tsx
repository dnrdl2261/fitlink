...
export default function CommunityPostScreen() {
  ...
  const goTag = (tag: string) =>
    router.navigate({ pathname: '/(trainer)/community', params: { tag, t: String(Date.now()), ...(from && from !== 'post' ? { from } : {}) } } as any);

  const handleSubmitComment = () => {
    const text = commentText.trim();
    if (!text) return;
    addComment(post.id, text, trainer?.name ?? '익명', trainer?.profileImageUrl, trainer?.id);
    setCommentText('');
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.header}>
          <TouchableOpacity onPress={() => router.navigate((from === 'post' && returnPostId ? { pathname: '/(trainer)/community-post', params: { postId: returnPostId } } : { pathname: '/(trainer)/community', params: from && from !== 'post' ? { from } : {} }) as any)} style={styles.backBtn}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.groupRow}>
                {matchingGroups.map(g => (
                  <TouchableOpacity key={g.id} style={styles.groupCard} activeOpacity={0.8}
                    onPress={() => router.push({ pathname: '/(trainer)/community-group', params: { groupId: g.id, from: 'post', returnPostId: post.id } } as any)}>
              ...
              {similarPosts.map(p => {
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => router.push({ pathname: '/(trainer)/community-post', params: { postId: p.id, from: 'post', returnPostId: post.id } } as any)}
                  >
...
const styles = StyleSheet.create({
  ...
  authorInitial: { fontSize: 16, fontWeight: '700', color: COLORS.secondary },
  ...
  commentInitial: { fontSize: 13, fontWeight: '700', color: COLORS.secondary },
  ...
  popularRankHot: { color: COLORS.secondary },
});
