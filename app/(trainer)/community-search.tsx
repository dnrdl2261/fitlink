...
export default function TrainerCommunitySearchScreen() {
  ...
  const goPost = (post: Post) =>
    router.push({
      pathname: post.isVideo ? '/(trainer)/community-story' : '/(trainer)/community-post',
      params: withFrom({ postId: post.id }),
    } as any);
  const goGroup = (groupId: string) =>
    router.push({ pathname: '/(trainer)/community-group', params: withFrom({ groupId }) } as any);
