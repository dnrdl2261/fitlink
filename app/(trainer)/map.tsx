...
export default function TrainerMapScreen() {
  ...
  useEffect(() => {
    ...
    gyms.forEach(gym => {
      ...
      el.style.cssText = [
        `background:${gym.isPartner ? COLORS.gym : '#606060'}`,
        'color:#fff',
        'width:28px',
        'height:28px',
        'border-radius:50%',
    ...
    Object.entries(markerEls.current).forEach(([gymId, el]) => {
      ...
      if (sel) {
        el.style.cssText = [
          ...
          'white-space:nowrap',
          'cursor:pointer',
          `box-shadow:0 4px 14px ${COLORS.secondary}55`,
  ...
  const renderMarkers = useCallback(() => {
    ...
    clusterByGrid(points).forEach(c => {
      ...
      const el = document.createElement('div');
      el.style.cssText = clusterBubbleCss(c.ids.length, COLORS.gym);
      ...
      <View style={styles.container}>
        ...
        <SafeAreaView pointerEvents="box-none" style={styles.topOverlay}>
          <View style={styles.topBar}>
            <View style={styles.locChip}>
              {hasPermission === null
                ? <><ActivityIndicator size="small" color={COLORS.secondary} style={{ marginRight: 5 }} /><Text style={styles.locChipText}>위치 확인 중</Text></>
                : hasPermission
            ...
            <View style={styles.legendChip}>
              <View style={[styles.legendDot, { backgroundColor: COLORS.gym }]} />
              <Text style={styles.legendText}>파트너</Text>
              <View style={[styles.legendDot, { backgroundColor: '#606060' }]} />
        ...
        {selectedGym && (
          <TouchableOpacity
            ...
            onPress={() => router.push({ pathname: '/gym/[id]', params: { id: selectedGym.id, from: 'trainer' } } as any)}
            activeOpacity={0.88}
          >
      ...
      >
        {gyms.map(gym => (
          <Marker key={gym.id} coordinate={gym.coordinate}
            ...
            pinColor={gym.id === selectedGymId ? COLORS.secondary : (gym.isPartner ? COLORS.gym : '#888')}
            title={gym.name}
          />
        ))}
      </MapView>
...
const styles = StyleSheet.create({
  ...
  selDist: { fontSize: 12, color: COLORS.secondary, fontWeight: '600' },
  ...
  partnerBadge: { backgroundColor: COLORS.gym, borderRadius: 5, paddingHorizontal: 6, paddingVertical: 2 },
  ...
  gymRowSel: { backgroundColor: COLORS.secondary + '0e', borderLeftWidth: 3, borderLeftColor: COLORS.secondary, paddingLeft: 13 },
