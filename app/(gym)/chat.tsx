...
export default function GymChatScreen() {
  ...
  const startWithTrainer = (trainerId: string, trainerName: string) => {
    if (!gymAdmin) return;
        ...
        {conversations.length === 0 && (
          <View style={styles.empty}>
            <View style={styles.emptyIconBox}>
              <MaterialCommunityIcons name="chat-outline" size={36} color={SLATE} />
            </View>
        ...
        {conversations.map((conv) => {
          ...
          return (
            <TouchableOpacity key={conv.id} style={styles.convCard} onPress={() => openChat(conv.id)}>
              <View style={styles.convCardBar} />
              <View style={styles.convCardInner}>
                <View style={[styles.convAvatar, { backgroundColor: GYM + '18' }]}>
                  <MaterialCommunityIcons name="dumbbell" size={22} color={GYM} />
                </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              {MOCK_TRAINERS.map((t) => (
                <TouchableOpacity
                  ...
                  onPress={() => startWithTrainer(t.id, t.name)}
                >
                  <View style={[styles.trainerAvatar, { backgroundColor: GYM + '18' }]}>
...
const styles = StyleSheet.create({
  ...
  emptyText: { fontSize: 16, fontWeight: '700', color: DARK },
  emptySubText: { fontSize: 13, color: '#94A3B8', fontWeight: '500' },
  ...
  convCardBar: { width: 4, backgroundColor: GYM },
  convCardInner: {
