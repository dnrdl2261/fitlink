...
export default function CommunityPostScreen() {
  ...
  const goTag = (tag: string) =>
    router.navigate({ pathname: '/(gym)/community', params: { tag, t: String(Date.now()), ...(from && from !== 'post' ? { from } : {}) } } as any);
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.header}>
          <TouchableOpacity onPress={() => router.navigate((from === 'post' && returnPostId ? { pathname: '/(gym)/community-post', params: { postId: returnPostId } } : { pathname: '/(gym)/community', params: from && from !== 'post' ? { from } : {} }) as any)} style={styles.backBtn}>
