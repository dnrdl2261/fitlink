...
export default function CommunityWriteScreen() {
  ...
  const handleSubmit = async () => {
    ...
    let imgUrl = images[0];
    if (isLocalUri(imgUrl) && canUpload(gymAdmin?.id)) {
      const up = await uploadMedia(imgUrl, 'posts', gymAdmin!.id);
    ...
    if (mediaType === 'video' && videoUri) {
      if (!canUpload(gymAdmin?.id)) { notify('동영상 업로드 불가', '로그인 후 이용해주세요.'); return; }
      const upv = await uploadMedia(videoUri, 'posts', gymAdmin!.id);
    ...
    addPost({
      ...
      author: gymAdmin?.name ?? '익명',
      authorId: gymAdmin?.id,
      authorAvatar: gymAdmin?.profileImageUrl,
      location: '알 수 없음',
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerCenter}>
            ...
            <View style={styles.authorChip}>
              ...
              <Text style={styles.authorChipText}>{gymAdmin?.name ?? '익명'}</Text>
            </View>
          </View>
          <TouchableOpacity
            ...
            <View style={styles.mediaTypeRow}>
              ...
              >
                <MaterialCommunityIcons
                  ...
                  color={mediaType === 'image' ? '#2DD4BF' : COLORS.textSecondary}
                />
              ...
              >
                <MaterialCommunityIcons
                  name="video" size={16}
                  color={mediaType === 'video' ? '#2DD4BF' : COLORS.textSecondary}
                />
...
const styles = StyleSheet.create({
  ...
  mediaTypeBtnActive: { borderColor: '#2DD4BF', backgroundColor: '#2DD4BF' + '10' },
  mediaTypeText: { fontSize: 13, fontWeight: '600', color: COLORS.textSecondary },
  mediaTypeTextActive: { color: '#2DD4BF' },
