...
export default function MapScreen() {
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      try {
        ...
        // ⚠️ 마커는 여기서 만들지 않는다. initMap은 1회만 실행되므로
        //    검색·지도이동으로 나중에 추가된 헬스장은 마커가 안 생겼다
        //    (자동완성엔 뜨는데 지도엔 없는 문제). 아래 별도 effect에서 목록 변화에 맞춰 생성한다.
        setMapReady(true);
      } catch (e: any) {
  ...
  // 전국 1만6천 곳을 다 갖고 있지 않으므로, 검색어는 서버에서도 조회해 목록에 병합한다.
  useEffect(() => {
    const q = gymQuery.trim();
    if (q.length < 2) return;
    const t = setTimeout(() => { useGymStore.getState().searchGymsByName(q); }, 350);
  ...
  // 전국 데이터는 화면 범위·검색으로 나중에 들어오므로, 초기화 때 한 번 그리는 것으로는 부족하다.
  // 이미 만든 마커는 건너뛰고 새로 들어온 것만 만든다.
  useEffect(() => {
    ...
    gyms.forEach(gym => {
      if (markerEls.current[gym.id]) return;             // 이미 있음
      if (!gym.coordinate?.latitude || !gym.coordinate?.longitude) return; // 좌표 없는 행 방어
  ...
  // ── 웹: 마커 표시·숨김 + 겹치는 마커 묶기 (하이라이트 effect 뒤에 적용)
  ...
  const renderMarkers = useCallback(() => {
    ...
    const byId = new Map(displayGyms.map(g => [g.id, g]));
    // 선택한 헬스장은 묶지 않는다 — 선택 표시가 버블에 가려지면 뭘 골랐는지 알 수 없다.
    const points = displayGyms
      .filter(g => g.id !== selectedGymId && g.coordinate?.latitude && g.coordinate?.longitude)
      .map(g => {
    ...
    clusterByGrid(points).forEach(c => {
      if (c.ids.length < 2) return;                       // 혼자면 개별 핀 그대로
      c.ids.forEach(id => clustered.add(id));

      const el = document.createElement('div');
      el.style.cssText = clusterBubbleCss(c.ids.length, COLORS.primary);
    ...
    Object.entries(markerEls.current).forEach(([gymId, el]) => {
      el.style.display = byId.has(gymId) && !clustered.has(gymId) ? '' : 'none';
    });
            ...
            <TouchableOpacity style={styles.locChip} onPress={() => router.push('/location-picker' as any)} activeOpacity={0.8}>
              {hasPermission === null ? (
                ...
                <Text style={styles.locChipText} numberOfLines={1}>
                  {selectedDong ? `📍 ${selectedDong}` : hasPermission ? '📍 현재 위치 기준' : `⚠️ ${DEFAULT_LOCATION_LABEL} 중심 기준`} ▾
                </Text>
              )}
            </TouchableOpacity>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                {displayGyms.map(gym => (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                  >
                    <GymThumb name={gym.name} uri={gym.images[0]} size={56} radius={10} />
                    <View style={styles.gymInfo}>
                      ...
                      <View style={styles.gymMeta}>
                        {gym.reviewCount > 0 ? <StarRating rating={gym.rating} size="small" /> : <Text style={styles.noReview}>리뷰 없음</Text>}
