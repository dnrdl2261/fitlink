...
export default function CommunityPostScreen() {
  ...
  const handleSubmitComment = () => {
    ...
    if (!text) return;
    addComment(post.id, text, member?.name ?? '익명', member?.profileImageUrl, member?.id);
    setCommentText('');
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.header}>
          <TouchableOpacity onPress={() => router.navigate((from === 'post' && returnPostId ? { pathname: '/(member)/community-post', params: { postId: returnPostId } } : '/(member)/community') as any)} style={styles.backBtn}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.groupRow}>
                {matchingGroups.map(g => (
                  <TouchableOpacity key={g.id} style={styles.groupCard} activeOpacity={0.8}
                    onPress={() => router.push({ pathname: '/(member)/community-group', params: { groupId: g.id, from: 'post', returnPostId: post.id } } as any)}>
...
const styles = StyleSheet.create({
  ...
  authorInitial: { fontSize: 16, fontWeight: '700', color: COLORS.primary },
  ...
  popularRankHot: { color: COLORS.primary },
});
