...
export default function MemberCommunitySearchScreen() {
  ...
  const goPost = (post: Post) =>
    router.push({
      pathname: post.isVideo ? '/(member)/community-story' : '/(member)/community-post',
      params: withFrom({ postId: post.id }),
    } as any);
  const goGroup = (groupId: string) =>
    router.push({ pathname: '/(member)/community-group', params: withFrom({ groupId }) } as any);
