...
function LikeSheet({ post, onClose }: { post: Post; onClose: () => void }) {
  ...
  const names = iLiked ? [member?.name ?? '나', ...others] : others;
...
function CommentSheet({ postId, onClose }: { postId: string; onClose: () => void }) {
  ...
  const submit = () => {
    const t = text.trim();
    if (!t) return;
    addComment(postId, t, member?.name ?? '익명', member?.profileImageUrl, member?.id);
    setText('');
  };
...
export default function MemberCommunityScreen() {
  ...
  const goVideoWrite = () =>
    router.push({ pathname: '/(member)/community-write', params: { t: String(Date.now()), mode: 'video', ...(from ? { from } : {}) } } as any);
  ...
  const goSearch = () =>
    router.push({ pathname: '/(member)/community-search', params: { ...(from ? { from } : {}) } } as any);
