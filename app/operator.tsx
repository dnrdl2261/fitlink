...
// 실 트레이너 식별 (데모 카탈로그 trainer_001 등은 심사 대상이 아니다)
...
export default function OperatorConsole() {
  ...
  const reviewCertification = useTrainerStore((s) => s.reviewCertification);
  const [tab, setTab] = useState<'reports' | 'apps' | 'certs'>('reports');

  // 실 트레이너만 심사 대상. 데모 카탈로그(trainer_001 등)는 DB에 없고 전부 verified=true라 제외한다.
  // ⚠️ 셀렉터 안에서 filter/map 하면 매번 새 배열이라 무한 렌더가 난다 → useMemo로 파생.
  const certQueue = useMemo(
    () =>
      trainers
        ...
        .flatMap((t) =>
          (t.certifications ?? [])
            ...
            .map((c) => ({ trainer: t, cert: c }))
        ),
    [trainers]
  );

  // 운영자 콘솔은 로그인 시 트레이너 목록을 받지 않으므로 여기서 직접 불러온다.
  useEffect(() => {
    if (role === 'operator') useTrainerStore.getState().loadFromSupabase();
  }, [role]);
  ...
  return (
    <SafeAreaView style={s.c}>
      ...
      <View style={s.tabRow}>
        ...
        <TouchableOpacity style={[s.tab, tab === 'certs' && s.tabOn]} onPress={() => setTab('certs')}>
          <Text style={[s.tabText, tab === 'certs' && s.tabTextOn]}>자격 심사{certQueue.length > 0 ? ` (${certQueue.length})` : ''}</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }} showsVerticalScrollIndicator={false}>
        ...
        {tab === 'certs' && (
          <>
            <Text style={s.notice}>
              트레이너가 등록한 자격입니다. 자격증 사본을 고객센터로 받아 확인한 뒤 승인하세요.
              승인한 자격만 회원에게 &apos;✓ 인증&apos;으로 표시됩니다.
            </Text>
            {certQueue.length === 0 ? <Empty text="심사 대기 중인 자격이 없습니다" /> :
              certQueue.map(({ trainer, cert }) => (
                <View key={`${trainer.id}_${cert.id}`} style={s.card}>
                  <View style={s.cardTop}>
                    <Text style={s.cardTitle}>{cert.name}</Text>
                    <View style={[s.badge, { backgroundColor: '#F59E0B1A' }]}>
                      <Text style={[s.badgeText, { color: '#F59E0B' }]}>대기</Text>
                    </View>
                  </View>
                  <Text style={s.cardReason}>발급 {cert.issuer || '미기재'}</Text>
                  <Text style={s.cardMeta}>
                    트레이너 {trainer.name} · 취득 {cert.issuedDate || '미기재'}
                    {cert.expiryDate ? ` · 만료 ${cert.expiryDate}` : ''}
                  </Text>
                  <View style={s.actionRow}>
                    <ActBtn label="승인" color="#10B981" onPress={() => reviewCertification(trainer.id, cert.id, true)} />
                    <ActBtn label="반려" color="#EF4444" onPress={() => reviewCertification(trainer.id, cert.id, false)} />
                  </View>
                </View>
              ))}
          </>
...
const s = StyleSheet.create({
  ...
  notice: { fontSize: 12.5, color: COLORS.textSecondary, lineHeight: 19, marginBottom: 14 },
