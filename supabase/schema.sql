...
-- ============================================================
-- Phase R: 트레이너 자격 심사 (운영자 승인)
-- ------------------------------------------------------------
-- 트레이너가 등록한 자격은 항상 verified=false 로 들어간다(앱에서 강제).
-- 운영자가 확인한 뒤에만 true 가 되도록, 운영자에게 trainers UPDATE 권한을 준다.
--
-- ⚠️ 기존 "trainers update self" 정책은 본인(profile_id = auth.uid())만 허용이라
--    운영자가 남의 자격을 승인할 수 없었다. 정책을 추가로 얹는다(정책은 OR로 합쳐진다).
--
-- ⚠️ 운영자도 UPDATE만 준다. INSERT 는 주지 않는다 —
--    클라이언트가 upsert 를 쓰면 INSERT 권한을 요구해 막히므로,
--    앱은 반드시 update().eq('id', ...) 로 호출해야 한다(trainerStore.reviewCertification).
-- ============================================================
drop policy if exists "trainers update operator" on public.trainers;
create policy "trainers update operator" on public.trainers for update
  using (public.is_operator()) with check (public.is_operator());

-- ------------------------------------------------------------
-- 자기 자격 승인 차단 트리거
-- 앱에서는 자격 등록 시 verified=false 로 넣지만, 그건 클라이언트 코드일 뿐이라
-- 트레이너가 REST로 직접 verified=true 를 써 넣으면 그대로 들어간다(본인 행 UPDATE 권한 보유).
-- 운영자가 아니면 verified·reviewedAt 을 이전 값으로 되돌려, 뱃지를 스스로 켤 수 없게 한다.
-- ------------------------------------------------------------
create or replace function public.protect_cert_verified()
returns trigger language plpgsql security definer set search_path = public as $$
declare
  prev_certs jsonb := case when tg_op = 'UPDATE' then coalesce(old.certifications, '[]'::jsonb) else '[]'::jsonb end;
  merged jsonb := '[]'::jsonb;
  c jsonb;
  prev jsonb;
begin
  ...
  for c in select * from jsonb_array_elements(coalesce(new.certifications, '[]'::jsonb)) loop
    select e into prev from jsonb_array_elements(prev_certs) e where e->>'id' = c->>'id' limit 1;
    merged := merged || jsonb_build_array(
      jsonb_set(
        jsonb_set(c, '{verified}',   coalesce(prev->'verified',   'false'::jsonb), true),
        '{reviewedAt}', coalesce(prev->'reviewedAt', 'null'::jsonb), true
      )
    );
  end loop;

  new.certifications := merged;
  return new;
end $$;

drop trigger if exists trg_protect_cert_verified on public.trainers;
create trigger trg_protect_cert_verified before insert or update on public.trainers
  for each row execute function public.protect_cert_verified();
